...
| Clicking on the **Monitor** output | The monitor rides ~64 ms behind the live path by design and declicks its gaps; persistent clicking means the cushion is still losing the race — raise `block_size` as above, and prefer a WASAPI entry for the monitor device. |
